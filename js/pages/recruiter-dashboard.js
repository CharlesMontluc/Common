...
async function loadDashboard() {
    try {
        ...
        for (const offer of offers) {
            const apps = await ApplicationsService.getOfferApplications(offer.id);
...
document.addEventListener('DOMContentLoaded', async () => {
    document.getElementById('companyName').textContent = authManager.currentUserProfile?.company || 'Recruiter';
    await loadDashboard();
});
